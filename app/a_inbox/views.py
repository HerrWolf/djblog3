...
def search_user(request):
    ...
    if request.htmx:
        if len(letters) > 0:
            ...
            users = User.objects.filter(Q(username__icontains=letters) | Q(id__in=users_id))
...
def new_message(request, recipient_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Encrypt message
            message.body = f.encrypt(message.body.encode('utf-8')).decode('utf-8')
    ...
    return render(request, 'a_inbox/new_message.html', context)
